...
test("postBridgeStatus sends heartbeat payload", async () => {
  let captured;
  const client = new PythonServiceClient({
    baseUrl: "http://python.local",
    fetchImpl: async (url, init) => {
  ...
  await client.postBridgeStatus({ voiceChannelId: "channel", driverAudioPackets: 3 });

  assert.equal(captured.url, "http://python.local/api/discord/bridge-status");
  assert.equal(captured.init.method, "POST");
  assert.equal(
    captured.init.body,
    JSON.stringify({ voiceChannelId: "channel", driverAudioPackets: 3 })
  );
});
