...
def test_ask_includes_conversation_context_when_provided():
    client = OpenAICompatibleClient(
        LLMConfig(base_url="http://127.0.0.1:8000/v1", model="test-model")
    )
    captured = {}
    ...
    client.ask(
        "why",
        RaceSnapshot(),
        conversation_context={"intent": "pit_status", "response": "Fuel tight."},
    )

    user_content = json.loads(captured["payload"]["messages"][1]["content"])
    assert user_content["conversation_context"]["intent"] == "pit_status"
    assert user_content["conversation_context"]["response"] == "Fuel tight."
